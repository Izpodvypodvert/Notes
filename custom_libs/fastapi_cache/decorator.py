...
from custom_libs.fastapi_cache.coder import Coder
...
def cache(
    ...
    def wrapper(func: Callable[P, Awaitable[R]]) -> Callable[P, Awaitable[R]]:
        ...
        @wraps(func)
        async def inner(*args: P.args, **kwargs: P.kwargs) -> Awaitable[R] | R:
            nonlocal coder
            nonlocal expire
            nonlocal key_builder

            async def ensure_async_func(*args: P.args, **kwargs: P.kwargs) -> Awaitable[R] | R:
            ...
            request: Optional[Request] = cast(
                Optional[Request], copy_kwargs.pop("request", None))
            response: Optional[Response] = cast(
                Optional[Response], copy_kwargs.pop("response", None))

            if (
                request and request.headers.get(
                    "Cache-Control") in ("no-store", "no-cache")
